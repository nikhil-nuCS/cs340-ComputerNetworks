...
def create_tables(input_filename, output_filename):
    ...
    # Load the JSON stored by first part
...
def write_to_file(content, filename):
    ...
    # Writing to file all the tables and compiling a report
...
def create_report_1(data):
    ...
    table_headers = ["Website Name", "Scan Time", "IPv4", "IPv6", "Server", "Redirect to HTTPS", "HSTS",
                     "Insecure HTTP", "TLS", "Root CA", "RDNS Names", "RTT range (ms)", "Locations"
                     ]
    report_table_1.set_cols_align(['c'] * len(table_headers))

    report_table_1.header(table_headers)

    # Parsing each website details and creating a tuple to add into the table
...
def create_report_2(data):
    ...
    table_headers = ["Website Name", "RTT Range (ms)"]
    report_table_2.set_cols_width([20, 15])
    report_table_2.set_cols_align(['c'] * len(table_headers))
    report_table_2.header(table_headers)
    tuples = []
    for each_website in data:
        website_details = data[each_website]
        # Creating a tuple with format (website,[rtt_range]) to simplify sorting according to the requirements
        tuples.append((each_website, website_details["rtt_range"]))

    # Sorting the rtt_min values in ascending order
...
def create_report_5(data):
    ...
    # Total number of domains scanned by the program
    ...
    # Calculating % values w.r.t. total_count and appending it in table
